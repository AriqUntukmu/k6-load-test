...
// Load payload data
const payloadData = JSON.parse(open('./payload-persona.json'));
...
function testRobynAPI() {
  ...
  group('Robyn - Persona Analysis', () => {
    robynAttempts.add(1);
    console.log(`[${new Date().toISOString()}] Attempting Robyn request...`);
    const response = http.post(`${ROBYN_BASE_URL}/api/persona/somnia`, JSON.stringify(payloadData), params);
    console.log(
...
function testFastAPI() {
  ...
  group('FastAPI - Persona Analysis', () => {
    fastApiAttempts.add(1);
    console.log(`[${new Date().toISOString()}] Attempting FastAPI request...`);
    const response = http.post(`${FASTAPI_BASE_URL}/sorting-hat`, JSON.stringify(payloadData), params);
    console.log(
