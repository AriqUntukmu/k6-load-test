...
// Load payload data
const payloadData = JSON.parse(open('./payload-dna.json'));
...
export const options = {
  ...
  thresholds: {
    http_req_duration: ['p(95)<20000'], // 95% of requests should be below 10000ms (relaxed for initial testing)
    http_req_failed: ['rate<0.1'], // Error rate should be less than 10% (relaxed for initial testing)
    robyn_response_time: ['p(95)<20000'],
    fastapi_response_time: ['p(95)<20000'],
...
function testRobynAPI() {
  ...
  group('Robyn - DNA Analysis', () => {
    robynAttempts.add(1);
    console.log(`[${new Date().toISOString()}] Attempting Robyn request...`);
    const response = http.post(`${ROBYN_BASE_URL}/api/dna/generate`, JSON.stringify(payloadData), params);
    console.log(
    ...
    const checks = check(response, {
      ...
      'Robyn response time < 20000ms': (r) => r.timings.duration < 20000,
...
function testFastAPI() {
  ...
  group('FastAPI - DNA Analysis', () => {
    fastApiAttempts.add(1);
    console.log(`[${new Date().toISOString()}] Attempting FastAPI request...`);
    const response = http.post(`${FASTAPI_BASE_URL}/digital-dna-v3`, JSON.stringify(payloadData), params);
    console.log(
    ...
    const checks = check(response, {
      ...
      'FastAPI response time < 20000ms': (r) => r.timings.duration < 20000,
